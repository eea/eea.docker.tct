...
from .factories import UserFactory, NationalObjectiveFactory

pattern = r"\s*[,;/\\]\s*"


class TestNationalObjectives(WebTest):

    client = Client()
    pytestmark = pytest.mark.django_db

    def test_list_objectives(self):
        self.obj = NationalObjectiveFactory()
        res = self.app.get('/objectives')
        assert res.status_int == 200
        assert res.html.h1.text == 'Objective {0}: {1}'.format(
                                            self.obj.code,
                                            self.obj.title)

    def test_get_objective(self):
        self.obj = NationalObjectiveFactory()
        res = self.app.get('/objectives/{0}'.format(self.obj.code))
        assert res.status_int == 200
        assert res.html.h1.text == 'Objective {0}: {1}'.format(
                                            self.obj.code,
                                            self.obj.title)
    ...
    def test_edit_objective(self):
        ...
        resp = self.app.get(reverse('edit_national_objective',
...
#         super(NationalObjectiveTestCase, self).__init__(*args, **kwargs)
